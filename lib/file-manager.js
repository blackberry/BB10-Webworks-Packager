...
function generateFrameworkModulesJS(session) {
    var libFiles = wrench.readdirSyncRecursive(path.normalize(session.sourceDir + barConf.LIB)),
        modulesList = [],
        modulesStr = "var frameworkModules = ";

    libFiles.forEach(function (file) {
        file = path.resolve(session.sourceDir + barConf.LIB, file);

        if (!fs.statSync(file).isDirectory()) {
            modulesList.push(path.relative(path.normalize(session.sourceDir + barConf.CHROME), file).replace(/\\/g, "/"));
        }
    });

    modulesStr += JSON.stringify(modulesList) + ";";
    fs.writeFileSync(path.normalize(session.sourceDir + barConf.CHROME + "/frameworkModules.js"), modulesStr);
}
...
module.exports = {
    ...
    cleanSource: function (session) {
        wrench.rmdirSyncRecursive(session.sourceDir); // TODO this is causing "UNKNOWN" exception
    }
};

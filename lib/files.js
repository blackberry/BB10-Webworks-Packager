...
module.exports = {
    prepare: function (session, features) {
        // copy framework
        fstools.copy(conf.LIB, path.normalize(session.sourceDir + bar_conf.LIB));

        // copy bootstrap
        fstools.copy(conf.DEPENDENCIES_BOOTSTRAP, path.normalize(session.sourceDir + bar_conf.CHROME));

        // unzip archive
        unzip(session.archivePath, session.sourceDir);

        // copy extensions
        copyExtensions(features, conf.EXT, session.sourceDir + bar_conf.EXT);
    },

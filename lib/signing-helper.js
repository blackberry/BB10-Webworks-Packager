...
function getKeyStoreDefaultPath() {
    ...
    if (os.type().toLowerCase().indexOf("windows") >= 0) {
        ...
        p = process.env.HOMEDRIVE + process.env.HOMEPATH + "\\Local Settings\\Application Data\\Research In Motion\\" + authorP12;
        if (path.existsSync(p)) {
            return p;
        }

        // Try Windows Vista and Windows 7 location
        p =  process.env.HOMEDRIVE + process.env.HOMEPATH + "\\AppData\\Local\\Research In Motion\\" + authorP12;
        if (path.existsSync(p)) {
            return p;
        }

...
function createAccessListObj(featuresArray, uri, allowSubDomain) {
    ...
    if (featuresArray) {
        for (j = 0; j < featuresArray.length; j++) {
            accessObj.features.push(featuresArray[j]["@"]);
        }
    }
...
function processWidgetData(data, widgetConfig) {
    ...
    if (data.feature) {
        ...
        localFeatures = createAccessListObj(packager_utils.isArray(data.feature) ? data.feature : [data.feature], "WIDGET_LOCAL", true);
    ...
    if (data.access) {
        ...
        if (!packager_utils.isArray(data.access)) {
            data.access = [data.access];
        }
    
        for (i = 0; i < data.access.length; i++) {
            attribs = data.access[i]["@"];
            
            if (attribs && data.access[i]) {
                widgetConfig.accessList.push(createAccessListObj(data.access[i].feature, attribs.uri, attribs.subdomains));
            }
        }
    }
}
